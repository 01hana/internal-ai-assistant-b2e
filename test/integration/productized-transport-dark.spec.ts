...
describe('dark central transport to real Customer-local Phase 6 route',()=>{
  it('authenticates exact signed bytes and validates the genuine correlated binding failure',async()=>{
    ...
    const module=await Test.createTestingModule({imports:[ProductizedBusinessConnectorModule.register({environment,allowTestLoopbackTls:true,resolver:async()=>[{address:'127.0.0.1',family:4}],requestFactory:(options,callback)=>httpsRequest({...options,ca:cert},callback)})]}).compile();
    ...
    try{
      ...
      const wrongModule=await Test.createTestingModule({imports:[ProductizedBusinessConnectorModule.register({environment:wrongHost,allowTestLoopbackTls:true,resolver:async()=>[{address:'127.0.0.1',family:4}],requestFactory:(options,callback)=>httpsRequest({...options,ca:cert},callback)})]}).compile();
