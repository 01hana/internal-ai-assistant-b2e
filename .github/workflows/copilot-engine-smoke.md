---
name: Copilot Engine Read-only Smoke Test
on:
  workflow_dispatch:
permissions:
  contents: read
  ...
  scripts:
    smoke-summary:
      ...
      script: |
        ...
        if (item.engine !== "Copilot" || item.status !== "SMOKE_TEST_PASS") {
...
`smoke_summary` 的所有欄位都必須提供，`engine` 固定為 `Copilot`、`status` 固定為
