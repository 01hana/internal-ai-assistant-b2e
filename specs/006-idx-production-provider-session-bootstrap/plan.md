...
2. Document the Host/SDK integration contract only: Host current native AccessToken → `POST /api/v1/identity/exchange` with a provisioned selector → short-lived managed JWT → existing `POST /api/v1/assistant/sessions` → `sessionId`. The Host owns the opaque current-credential callback and native AccessToken/RefreshToken lifecycle; Feature 004 and Gateway perform verification, Customer/HostApp binding resolution, and internal Backend identity server-side.
3. Document managed credential re-exchange only: on expiry, obtain the current Host credential and repeat exchange. The new managed JWT may continue an existing authenticated session where existing identity/session authorization permits; re-exchange does not create, destroy, or receive a session ID. Do not invent retry, refresh-timer, storage, or SDK behavior.
4. Record deployment order: deploy additive migration and dormant capability; provision Provider Instance, exchange config/selector, Entry admission policy, provider-trusted permission policy, managed issuer/key, Feature 004 TrustProfile, and IntegrationBinding; execute readiness; enable each integration separately; then integrate and validate the Host callback/client flow in its target environment.
5. Roll back by disabling/replacing the IDX Provider Instance or exchange configuration/policies. Existing direct Feature 004, generic delegated provider, and prior `allow_empty`/`required` integrations remain operational; do not remove enum values or alter existing configuration during rollback.

The client never selects the MenuDetail endpoint, sends permission mapping or Customer authority, or forwards native/managed credentials to Backend. `integrationSelector` selects only server-owned configuration; exact Entry admission remains authoritative. `sessionId` is a conversation identifier, never authentication or authorization. IDX provider capability readiness remains distinct from any Customer production deployment, which additionally requires its own safely provisioned endpoint, control-plane records, Host callback integration, and target-environment validation.
