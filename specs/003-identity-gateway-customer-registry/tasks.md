...
- [x] T005 Run and classify the focused Feature 002 identity, CustomerScope, A/B no-disclosure, SSE, audit, and redaction baseline suites. **說明**: preserve accepted behavior; **輸出**: baseline evidence; **完成條件**: unrelated failures are recorded before Gateway work; **依賴**: T002–T004; **驗證**: existing `npm run test:unit`/contract subsets.
- [x] T006 Complete the Phase-0 checkpoint in `specs/003-identity-gateway-customer-registry/tasks.md` evidence section during implementation. **說明**: gate runtime work; **輸出**: modified-file/command/result/skip/blocker record; **完成條件**: T002–T005 pass; **依賴**: T005; **驗證**: checkpoint review.

**Checkpoint**: Feature 002 identity boundary is guarded; Gateway runtime work may begin, but no Gateway feature exists yet.

### Phase 0 Implementation Evidence — 2026-08-08

- **Authority inventory**: Backend-owned `RemoteJwksInternalIdentityTokenVerifier`, `IdentityGuard`, `validateVerifiedInternalIdentityClaims`, `RequestIdentityContext`, `CustomerScope`, request-ID middleware, `AuditWriterService`, centralized `redactSecrets()`, `src/main.ts`/`AssistantController` protected routes, and signed Customer A/B fixtures remain the only authority. Ignored `apps/gateway/dist/**` was not used.
- **Modified files**: Phase-0 identity/session, architecture, logger, and observability tests; `src/common/logger/redaction.util.ts`; this evidence record. No Gateway/package/schema/migration files changed.
- **T004 remediation**: centralized redaction now treats semantic private-JWK contexts and private-key field names as sensitive, redacts private PEM values under neutral keys, preserves ordinary non-security `d`/`p`/`q`, and retains safe Error code/request/trace behavior.
- **Commands/results**: `npx jest test/unit/logger-redaction.spec.ts test/unit/observability-metadata.spec.ts --runInBand --config jest.config.ts` → 2 suites / 15 tests PASS; focused non-socket unit/static batch → 7 suites / 62 tests PASS; `RUN_CUSTOMER_US1_TESTS=true` focused sandbox-external HTTP batch (identity, A/B session, audit/redaction, session/SSE contracts) → 7 suites / 57 tests PASS. All batches: 0 failed, 0 skipped.
- **Blockers**: none. Phase 1 remains unstarted pending human acceptance of this checkpoint.

---

## Phase 1 — Shared Contract, Gateway Skeleton, and Generation Foundation

- [x] T007 [P] Add pure-package surface tests in `test/unit/internal-identity-contract-package.spec.ts`. **說明**: require only claim names/types, registered metadata, and validation-neutral vocabulary; **輸出**: failing package contract; **完成條件**: Nest/Prisma/signer/verifier/CustomerScope/repository/authorization exports are rejected; **依賴**: T006; **驗證**: focused unit suite.
...
- [x] T009 [P] Add package/build/schema architecture guards in `test/unit/feature003-package-architecture.spec.ts`. **說明**: reject workspace conversion, TS paths/project references, runtime cross-imports, and Gateway-local Prisma schema/migrations; **輸出**: failing static guard; **完成條件**: root schema remains sole source; **依賴**: T006; **驗證**: non-socket unit suite.
- [x] T010 Implement `packages/internal-identity-contract/{package.json,tsconfig.json,src/index.ts}` and its declarations/exports. **說明**: create the pure local package; **輸出**: buildable vocabulary package; **完成條件**: T007 passes without authority runtime; **依賴**: T007; **驗證**: package build command planned here and T007.
- [x] T011 Modify the Gateway manifest created by T008, plus root `package.json`, `package-lock.json`, and Gateway build scripts, to add the local `file:` contract dependency and explicit build ordering. **說明**: the contract package builds before root-schema Prisma generation then Backend/Gateway builds; **輸出**: non-workspace dependency wiring; **完成條件**: both apps resolve package exports/declarations without recreating the T008 test harness; **依賴**: T008, T010; **驗證**: install/build resolution checks.
- [x] T012 Create the first Gateway production runtime skeleton in `apps/gateway/{tsconfig.json,nest-cli.json,src/main.ts,src/gateway.module.ts}`. **說明**: independent Nest bootstrap only; the T008 manifest/test harness already exists and is not recreated here; **輸出**: isolated app structure; **完成條件**: no Backend runtime import and no issuance path; **依賴**: T008, T011; **驗證**: Gateway build/start smoke.
- [x] T013 Implement fail-closed config and safe health/readiness in `apps/gateway/src/config/**` and `apps/gateway/src/health/**`. **說明**: use T008 contracts; **輸出**: validated configuration boundary; **完成條件**: missing/invalid config blocks bootstrap without leaks; **依賴**: T008, T012; **驗證**: Gateway config/health tests.
- [x] T014 Add the second Gateway generated-client output to root `prisma/schema.prisma` and root generation wiring only. **說明**: one schema produces both clients; **輸出**: second generator configuration; **完成條件**: no `apps/gateway/prisma/**`; **依賴**: T009, T012; **驗證**: `prisma generate` and architecture guard.
- [x] T015 Verify shared-package, root Prisma generation, Backend build, Gateway build/start, and no-cross-import guards. **說明**: integration evidence; **輸出**: Phase-1 verification log; **完成條件**: all T007–T014 contracts pass; **依賴**: T013–T014; **驗證**: current commands plus planned Gateway commands finalized here.
- [x] T016 Complete the Phase-1 checkpoint evidence. **說明**: record foundation outcome; **輸出**: files/commands/results/skips/blockers; **完成條件**: T015 green; **依賴**: T015; **驗證**: checkpoint review.

**Checkpoint**: package and two independent app build paths exist; no identity issuance or Customer resolution exists.

### Phase 1 Implementation Evidence — 2026-08-09

- **Tests-first evidence**: T007/T009 initially failed only because `packages/internal-identity-contract` did not exist; T008's local Jest runner executed 4 tests and failed only because the config/health production modules did not exist. After implementation, root package/architecture plus affected identity suites passed 5 suites / 45 tests, and Gateway-local config/health passed 2 suites / 4 tests.
- **Foundation outputs**: added the pure private `@internal-ai-assistant/internal-identity-contract` local package; an independent Gateway Nest bootstrap with typed fail-closed configuration and process-only health/readiness; local `file:` dependency/build scripts; and a second Gateway Prisma client generator from the sole root schema. The Backend validator now consumes only the shared canonical claim-name vocabulary; its claim semantics and context/scope behavior are unchanged.
- **Verification**: `npx prisma validate`, `npm run prisma:generate`, `npm run build:feature003`, and `npm run typecheck` passed. The build order is contract → root Prisma clients → Backend → Gateway. A bounded localhost Gateway smoke returned safe `/health` and non-production `/readiness` responses; `RUN_CUSTOMER_US1_TESTS=true` focused Feature 002 HTTP regression passed 7 suites / 57 tests. Changed-file ESLint and package resolution from both root and Gateway paths passed.
- **Scope and blockers**: no skips or blockers. No workspace, TS paths, project references, Gateway-local Prisma schema/migrations, Phase 2 model, migration, upstream verification, binding resolution, signing, JWKS, or Backend transport was created. Phase 1 is complete; stop before T017 pending human acceptance.
- **Pre-Phase-2 cleanup — 2026-08-09**: T009 now parses all three manifests, derives the Backend package name from the root manifest, rejects every Gateway dependency section that references the Backend/root runtime or a non-contract local `file:` target, and statically rejects Gateway imports of that dynamic package name. Synthetic manifests prove a `file:../..` Backend bypass and a `file:../../src` bypass fail the guard. T007 now distinguishes registered JWT payload claims (`iss`, `aud`, `iat`, `exp`, `nbf`) from protected-header `kid`; `RS256` and canonical claim vocabulary remain pure exports. T008 accepts opaque local/provider references including `.pem` paths while rejecting blank/control/multiline input, raw private PEM, Bearer credentials, and compact JWT-shaped material through the same generic error. Verification: root targeted guard/package/identity batch 4 suites / 32 tests PASS; Gateway config/health 2 suites / 16 tests PASS; affected Feature 002 identity regression 2 suites / 36 tests PASS; contract build, Prisma generation, Feature 003 build, typecheck, and changed-file ESLint PASS. No skips or blockers; T017 remains unstarted.

---

## Phase 2 — Additive Persistence and Controlled Provisioning

- [x] T017 [P] Add IntegrationBinding persistence/FK/index tests in `test/integration/gateway-integration-binding.persistence.spec.ts`. **說明**: explicit existing Customer, unique integration identity, allowedHostApp/enabled, restrict deletion; **輸出**: failing DB contract; **完成條件**: one integration cannot bind two Customers; **依賴**: T016; **驗證**: DB-backed Jest suite.
- [x] T018 [P] Add GatewaySigningKey and GatewayIdentityAuditEvent persistence/redaction tests in `test/integration/gateway-signing-key.persistence.spec.ts` and `test/unit/gateway-identity-audit-redaction.spec.ts`. **說明**: key fields/states, public-only audit, at-most-one-active requirement; **輸出**: failing constraint/audit tests; **完成條件**: no JWT/Authorization/private JWK/key; **依賴**: T016; **驗證**: focused unit/integration suites.
- [x] T019 [P] Add controlled provisioning command tests in `test/integration/gateway-integration-provisioning.spec.ts`. **說明**: explicit Customer/integration/HostApp create, rerun, enable/disable/re-enable, conflict rejection; **輸出**: failing command contract; **完成條件**: no Customer creation/inference/public controller; **依賴**: T016; **驗證**: DB-backed suite.
- [x] T020 [P] Add synthetic A/B binding fixtures and shared-lower-level assertions in `test/support/gateway-identity-fixtures.ts` and `test/unit/gateway-identity-fixtures.spec.ts`. **說明**: share org/sub/host/roles/scopes but differ Customer/integration; **輸出**: deterministic fixtures; **完成條件**: no real keys/credentials; **依賴**: T016; **驗證**: fixture unit suite.
- [x] T021 Add `IntegrationBinding`, `GatewaySigningKey`, and `GatewayIdentityAuditEvent` models, relations, indexes, and enum fields to root `prisma/schema.prisma`. **說明**: additive canonical data model; **輸出**: root schema changes only; **完成條件**: Customer FK and key lifecycle fields match locked design; **依賴**: T017–T020; **驗證**: Prisma validate and failing tests advance.
- [x] T022 Create root `prisma/migrations/<timestamp>_feature003_gateway_registry/migration.sql` with additive tables/FKs/indexes and PostgreSQL partial unique enforcement for at most one active key. **說明**: do not assume Prisma schema expresses partial uniqueness; **輸出**: explicit SQL migration; **完成條件**: invalid active-key states reject; **依賴**: T021; **驗證**: T017–T018 migration tests and rollback inspection.
- [x] T023 Regenerate Backend and Gateway Prisma clients from root `prisma/schema.prisma`. **說明**: update generated outputs only through Prisma; **輸出**: typed generated clients; **完成條件**: no Gateway-local schema generated; **依賴**: T022; **驗證**: `prisma generate` and build checks.
- [x] T024 Implement `apps/gateway/src/{integration-registry/**,audit/**,commands/provision-integration-binding.ts}`. **說明**: repository, safe Gateway identity audit, and idempotent internal command; **輸出**: no public provisioning controller; **完成條件**: T019 passes and conflicting rebind fails closed; **依賴**: T019, T023; **驗證**: provisioning and audit tests.
- [x] T025 Add deterministic A/B binding seed support in `scripts/seed.ts` and `test/support/gateway-identity-fixtures.ts`. **說明**: explicit synthetic bindings only; **輸出**: repeatable A/B records; **完成條件**: no ownership inference; **依賴**: T020, T024; **驗證**: seed/fixture tests.
- [x] T026 Run persistence, provisioning, A/B fixture, Prisma migration, and affected Feature 002 Customer-ownership regression. **說明**: prove additive compatibility; **輸出**: Phase-2 verification evidence; **完成條件**: all DB contracts green; **依賴**: T024–T025; **驗證**: targeted DB suites and Feature 002 persistence tests.
- [x] T027 Complete the Phase-2 checkpoint evidence. **說明**: record outputs/results/skips/blockers; **輸出**: accepted registry foundation; **完成條件**: T026 green; **依賴**: T026; **驗證**: checkpoint review.
...
### Phase 2 Implementation Evidence — 2026-08-09

- **Tests-first**: T017/T018 RED was observed against an isolated safe PostgreSQL `_test` migration head because `IntegrationBinding` and `GatewaySigningKey` did not exist; T019/T018 RED was the absent controlled provisioning/audit production surface; T020 seed data was absent before Phase 2. No harness, JWT, DI, or listener error was involved.
- **Persistence**: root `prisma/schema.prisma` remains the only schema. Additive migration `20260809000000_feature003_gateway_registry` adds explicit `IntegrationBinding`, public-only `GatewaySigningKey`, and append-only `GatewayIdentityAuditEvent`; it includes restrictive Customer FKs and `GatewaySigningKey_one_active_key` partial unique SQL. Both Prisma clients were regenerated from root schema.
- **Provisioning and fixtures**: direct-only `ProvisionIntegrationBindingCommand` requires an existing explicit Customer, is idempotent for an exact replay, supports controlled enable/disable/re-enable, rejects Customer/HostApp rebind, and writes only safe whitelisted Gateway identity audit fields. Deterministic A/B seed bindings share lower upstream identity fixture values but differ Customer/integration; test reset deletes Gateway children before Customer without cascade.
- **Verification**: Phase 2 DB batch (`RUN_GATEWAY_REGISTRY_DB_TESTS=true`, `--detectOpenHandles`) passed 4 suites / 6 tests; architecture/identity/audit static batch passed 7 suites / 57 tests; Gateway unit/config-health batch passed 2 suites / 16 tests; Feature 002 persistence passed 2 suites / 13 tests and signed-JWT ownership/SSE/audit regression passed 4 suites / 33 tests. `prisma validate`, `prisma generate`, `build:feature003`, typecheck, and changed-file ESLint passed. No required skips or blockers.
- **Boundary**: Phase 2 establishes registry/key-public-metadata/security-audit persistence only. Upstream authentication, canonical Gateway identity, internal signing, JWKS, rotation, Backend transport, and production readiness remain unstarted; stop before T028.
- **Pre-Phase-3 cleanup — 2026-08-09**: aligned the uncommitted additive migration and canonical schema from non-authoritative `publishedAt` to locked-design nullable `notBefore`; regenerated both Prisma clients without a corrective migration. `GatewayIdentityAuditWriter` now permits only bounded lowercase ASCII safe codes (`^[a-z][a-z0-9_:-]{0,127}$`) for event type, outcome, and reason code while preserving identifier validation separately. Bearer/JWT/PEM-like safe-code values reject before Prisma create with the same generic error; provisioning’s created/replayed/updated/unknown-customer/conflict paths remain green. Verification: focused Phase 2 batch 6 suites / 14 tests PASS; architecture/package guards 2 suites / 9 tests PASS; Prisma validate/generate, Feature 003 build, typecheck, and changed-file TypeScript ESLint PASS. No test skips or failures. **Tooling blocker:** Prisma 7 generated output contains its own trailing whitespace, so the exact `git diff --check` reports generated-only whitespace findings; generated clients were left verbatim and not manually patched. T028 remains unstarted.

---

## Phase 3 — Upstream RS256 Authentication

- [x] T028 [US3] Add upstream RS256/JWKS/time failing tests in `apps/gateway/test/upstream-auth/upstream-token-verifier.spec.ts`. **說明**: signature, issuer/audience, `iat`/`exp`, optional `nbf`, malformed/future/expired/wrong-algorithm cases; **輸出**: verifier contract; **完成條件**: failures occur before registry/signing/Backend work; **依賴**: T027; **驗證**: Gateway unit suite.
- [x] T029 [P] [US3] Add verified upstream claim-shape/no-authority tests in `apps/gateway/test/upstream-auth/upstream-identity.spec.ts`. **說明**: required non-blank scalars, empty valid arrays, blank array rejection, public input conflicts; **輸出**: identity-shape contract; **完成條件**: no client augmentation; **依賴**: T027; **驗證**: Gateway unit suite.
- [x] T030 [P] [US7] Add upstream credential safe-error/redaction tests in `apps/gateway/test/upstream-auth/upstream-auth-redaction.spec.ts`. **說明**: redact raw upstream JWT/Authorization/claims; **輸出**: failure-surface contract; **完成條件**: generic denial only; **依賴**: T027; **驗證**: unit/logger tests.
- [x] T031 Implement deployment-configured upstream auth settings in `apps/gateway/src/config/gateway-config.service.ts`. **說明**: single issuer/audience/JWKS/tolerance; **輸出**: typed config; **完成條件**: invalid values fail at bootstrap; **依賴**: T028–T030; **驗證**: T028–T030.
- [x] T032 Implement verified-upstream types and RS256 Remote-JWKS verifier in `apps/gateway/src/upstream-auth/**`. **說明**: separate from binding/signing/Backend client; **輸出**: verified identity boundary; **完成條件**: no registry or signing dependency; **依賴**: T031; **驗證**: T028–T029.
- [x] T033 Wire safe upstream-auth exceptions and redacted telemetry in `apps/gateway/src/{upstream-auth/**,observability/**,audit/**}`. **說明**: 401/non-disclosing failure mapping; **輸出**: safe error path; **完成條件**: T030 passes; **依賴**: T032; **驗證**: redaction tests.
- [x] T034 Run upstream-auth targeted tests plus affected Feature 002 verifier, canonical-claim, and public-header regressions. **說明**: ensure no Backend contract drift; **輸出**: verification evidence; **完成條件**: all selected suites green; **依賴**: T033; **驗證**: Gateway and existing Backend Jest subsets.
- [x] T035 Complete the Phase-3 checkpoint evidence. **說明**: record results/skips/blockers; **輸出**: accepted upstream boundary; **完成條件**: T034 green; **依賴**: T034; **驗證**: checkpoint review.

**Checkpoint**: only a verified upstream identity may progress; it has neither Customer authority nor signing authority.

### Phase 3 Implementation Evidence — 2026-08-09

- **Tests-first and verifier**: T028–T030 RED was observed under a bounded local HTTP JWKS listener solely because the upstream-auth production surface was absent. Gateway now directly declares `jose@5.10.0` and uses `createRemoteJWKSet` plus `jwtVerify`, fixed RS256/protected `kid`, exact configured issuer/audience, and the existing single 0–300-second tolerance with strict `iat`/`exp`/optional `nbf` checks.
- **Verified identity boundary**: `VerifiedUpstreamIdentity` is frozen with frozen roles/scopes and contains only verified integration, subject, organization, HostApp, roles, and scopes. It ignores upstream `customer_id` and all extra claims; it imports no registry, Prisma, Customer, signing, JWKS-issuer, or Backend transport surface.
- **Safe failure and telemetry**: every public verification failure is generic `401 / UPSTREAM_IDENTITY_INVALID`; internal audit reason codes remain bounded safe codes. Failed-token telemetry writes only request/event/outcome/reason, while verified telemetry may use verified integration/actor/HostApp and never Customer. Raw Authorization, JWT, claims, signature, and credential sentinels do not escape to error or audit input.
- **Verification**: local HTTP JWKS upstream-auth batch passed 3 suites / 21 tests; full Gateway suite passed 5 suites / 37 tests; Feature 002 architecture/identity/header/redaction subset passed 7 suites / 70 tests. Feature 003 build, typecheck, and changed-file ESLint passed. No required skips or test blockers. Both exact `git diff --check` and the handwritten check excluding both generated Prisma trees passed. T036 remains unstarted.
- **Pre-Phase-4 cleanup — 2026-08-10**: Gateway issuer validation now preserves the trimmed configured `iss` string exactly for both internal and upstream issuers while JWKS and Backend endpoint URLs retain URL normalization. A real local-JWKS config → `GatewayConfigService` → verifier contract proves no-slash acceptance and slash mismatch denial. Public failures remain `401 / UPSTREAM_IDENTITY_INVALID`; detailed typed JOSE diagnostics are non-enumerable and never enter denied audit records, which always use `upstream_auth_invalid`. T029 now proves public headers/body/query/PageContext/metadata cannot affect verified upstream identity. Verification: focused upstream/config batch 4 suites / 40 tests, full Gateway 5 suites / 42 tests, Feature 002 identity/header/redaction subset 5 suites / 61 tests; `build:feature003`, typecheck, and changed-file ESLint PASS. No skips or blockers; T036 remains unstarted.

---

## Phase 4 — Integration-to-Customer Resolution and Canonical Identity

- [x] T036 [US1] Add resolver and no-inference failing tests in `apps/gateway/test/integration-registry/canonical-identity-resolver.spec.ts`. **說明**: unknown/disabled/missing binding, HostApp mismatch, no default Customer; **輸出**: resolver contract; **完成條件**: no signing/Backend call on denial; **依賴**: T035; **驗證**: Gateway unit suite.
- [x] T037 [P] [US1] Add A/B integration-isolation tests in `apps/gateway/test/integration-registry/customer-binding-isolation.spec.ts`. **說明**: shared lower identity with different binding Customer; **輸出**: cross-Customer denial contract; **完成條件**: A cannot obtain B and vice versa; **依賴**: T035, T025; **驗證**: Gateway DB integration suite.
- [x] T038 Implement immutable `CanonicalGatewayIdentity` types/composer in `apps/gateway/src/identity/**`. **說明**: bounded value from verified upstream identity plus binding only; **輸出**: frozen canonical identity; **完成條件**: Customer comes solely from binding; **依賴**: T036–T037; **驗證**: resolver tests.
- [x] T039 Implement Customer-qualified binding resolution in `apps/gateway/src/integration-registry/canonical-identity-resolver.service.ts`. **說明**: enabled, Customer FK, and HostApp checks; **輸出**: fail-closed resolver; **完成條件**: no org/sub/role/scope/header/body/PageContext/metadata inference; **依賴**: T038; **驗證**: T036–T037.
- [x] T040 [US3] Wire safe resolution-denial telemetry in `apps/gateway/src/{identity/**,audit/**,observability/**}`. **說明**: no resource-existence disclosure; **輸出**: safe issuance denial path; **完成條件**: no business audit or Backend request; **依賴**: T039; **驗證**: resolver/redaction tests.
- [x] T041 Run canonical resolution/A-B tests and affected Feature 002 CustomerScope, header non-authority, and no-disclosure regressions. **說明**: prove contract compatibility; **輸出**: verification evidence; **完成條件**: all selected suites green; **依賴**: T040; **驗證**: targeted Gateway/Backend suites.
- [x] T042 Complete the Phase-4 checkpoint evidence. **說明**: record results/skips/blockers; **輸出**: accepted canonical identity boundary; **完成條件**: T041 green; **依賴**: T041; **驗證**: checkpoint review.
...
### Phase 4 Implementation Evidence — 2026-08-10

- **Tests-first and authority**: T036 RED was the deliberately absent canonical resolver/composer surface; T037 RED reached a real disposable PostgreSQL `_test` database and failed only for that same absent surface. `CanonicalIdentityResolver` now performs one `findByIntegrationId(verifiedIdentity.integrationId)` lookup and maps Customer authority only from `IntegrationBinding.customerId`; it has no Customer, organization, subject, HostApp, role, scope, or public-hint lookup/fallback.
- **Canonical identity and denial**: frozen `CanonicalGatewayIdentity` contains only Customer, integration, subject, organization, HostApp, cloned frozen roles/scopes. It has no JWT metadata or `jti`. Unknown, disabled, malformed/mismatched binding and HostApp mismatch return generic `403 / IDENTITY_ISSUANCE_DENIED`; detailed subtype is non-enumerable. Denied Gateway identity audit is fixed to `identity_resolution_denied / denied / identity_issuance_denied`, stores only safe verified integration/actor/HostApp correlation fields, and never Customer ID or detailed subtype.
- **A/B PostgreSQL proof**: Customer A/B share organization, actor, HostApp, roles, and permission scopes, but real explicit bindings resolve only `integration-a → customer-a` and `integration-b → customer-b`; public B hints do not alter A, and disabled A never falls back to B. Temporary databases are disconnected and dropped by the shared safe helper.
- **Verification**: resolver unit 1 suite / 9 tests PASS; A/B PostgreSQL 1 suite / 2 tests PASS; full Gateway suite with DB/JWKS/open-handle diagnostics 7 suites / 53 tests PASS. Focused Feature 002/003 CustomerScope, header, no-disclosure, persistence, audit/redaction, provisioning, fixture, and architecture batch 15 suites / 102 tests PASS. `build:feature003`, typecheck, and changed-file ESLint PASS. No required skips, failures, schema/migration changes, or blockers; the root focused Jest run emits only the pre-existing non-blocking `ts-jest` built-local-package `allowJs` warning. T043 remains unstarted.
