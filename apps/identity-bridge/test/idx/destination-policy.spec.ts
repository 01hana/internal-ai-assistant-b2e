...
describe('Bridge destination policy', () => {
  it('permits only public-routable addresses in public_only mode', () => {
    ...
    for (const address of ['0.0.0.0', '127.0.0.1', '10.0.0.1', '100.64.0.1', '169.254.1.1', '224.0.0.1', '240.0.0.1', '::', '::1', '2001:db8::1', 'fc00::1', 'fe80::1', 'ff00::1', '::ffff:10.0.0.1']) expect(() => policy.assertAddresses([address])).toThrow('provider_unavailable');
  });
  ...
  it('admits only configured Customer CIDRs in allowlisted_networks mode and fails closed on mixed answers', () => {
    ...
    expect(() => policy.assertAddresses(['10.10.1.20', '192.168.1.10'])).toThrow('provider_unavailable');
    expect(() => policy.assertAddresses(['8.8.8.8'])).toThrow('provider_unavailable');
  });

  it('does not turn public, loopback, or special destinations into Customer-local targets through CIDR configuration', () => {
    expect(() => new BridgeDestinationPolicy(config({ IDX_DESTINATION_MODE: 'allowlisted_networks', IDX_ALLOWED_CIDRS: '8.8.8.0/24' }).configuration.destination).assertAddresses(['8.8.8.8'])).toThrow('provider_unavailable');
    ...
    expect(() => broad.assertAddresses(['10.10.1.20', '8.8.8.8'])).toThrow('provider_unavailable');
    expect(() => broad.assertAddresses(['127.0.0.1'])).toThrow('provider_unavailable');
    expect(() => broad.assertAddresses(['::1'])).toThrow('provider_unavailable');
  });
});
