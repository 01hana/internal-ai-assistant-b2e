...
export class ProviderContractValidatorRegistry {
  validate(providerType: string, version: string, contract: Contract): void {
    if (providerType !== 'delegated_http' || version !== 'delegated-http/v1') throw new ManagedExchangeActivationError();
  ...
  isActiveEligible(providerType: string, version: string): boolean { return providerType === 'delegated_http' && version === 'delegated-http/v1'; }
}
...
function assertKnownObject(value: Contract, keys: readonly string[]): void {
  if (!value || Object.keys(value).some((key) => !keys.includes(key))) throw new ManagedExchangeActivationError();
